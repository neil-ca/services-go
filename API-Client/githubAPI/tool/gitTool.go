...
// Repo is for holding response of repositories fetch API
type Repo struct {
	ID       int    `json:"id"`
...
// File is for modelling JSON body in create Gist
type File struct {
	...
	app.Commands = []*cli.Command{
		{
			...
			Action: func(c *cli.Context) error {
				if c.NArg() > 0 {
					...
					var repos []Repo
					user := c.Args()
					var repoUrl = fmt.Sprintf("http://api.github.com/users/%s/repos", user.Get(0))
		...
		},
		{
			...
			Action: func(c *cli.Context) error {
				if c.NArg() > 1 {
					...
					var postUrl = "http://api.github.com/gists"
